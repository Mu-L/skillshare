...
$_skillshareCompleter = {
    ...
    $flags = @{
        ...
        'doctor' = '--json', '--help', '-h'
        'target' = '--json', '--no-tui', '--help', '-h', '--mode', '--agent-mode', '--target-naming', '--add-include', '--add-exclude', '--remove-include', '--remove-exclude', '--add-agent-include', '--add-agent-exclude', '--remove-agent-include', '--remove-agent-exclude', '--agent', '--config-dir', '--project', '-p', '--global', '-g'
        ...
        'mcp' = '--pi-extension', '--direct-tools', '--pi-options', '--target', '--from', '--url', '--file', '--sync', '--replace', '--revision', '--dry-run', '--json', '--no-tui', '--help', '--project', '--global'
