...
| **Targets** | 상태가 표시된 target 목록. **대상 추가**는 **다른 계정**도 받습니다. 이미 사용 중인 Agent의 두 번째 config 폴더이며, 어디에 기록되는지 미리보기로 보여줍니다. 각 target의 페이지에서 include/exclude 필터를 편집하고 로컬 전용 skill을 source로 다시 수집합니다 |
| **프로젝트** | Global mode 전용. global 설정이 동기화하는 프로젝트 폴더이며, [`projects`](/docs/reference/targets/configuration#projects)와 [`mcp.projects`](./mcp.md#projects-in-the-dashboard)에서 옵니다. **프로젝트 추가**는 폴더, 그 target, 동기화할 항목을 받습니다. 각 프로젝트에는 필터, 미리보기, 기록될 폴더를 보여주는 **Skills**와 **Agents** 탭, 그리고 해당 폴더에서 global 서버를 끄거나 프로젝트 자체 서버를 부여하는 **MCP** 탭이 있습니다. 이미 프로젝트 폴더를 가리키는 Target은 변환할 수 있습니다 |
