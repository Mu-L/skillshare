...
| **MCP** | 每个 server 一行，以及它同步到的 Agents 开关。**Add server** 接受 URL、命令、粘贴的片段或文件；**Import** 会读取某个已安装 Agent 现有的配置。每个 server 的菜单都有 **View what each Agent gets**，用于显示每个 Agent 的原生配置，包括尚未保存的编辑。冲突会提供 Import 或 Replace 选项，Backup 也可以在恢复前预览。**默认值** 用于编辑 `mcp.targets` 和 `mcp.directTools`。在 global mode 下，**项目** 标签页用于管理 [`mcp.projects`](./mcp.md#projects-in-the-dashboard)：添加项目文件夹、在其中关闭 global server，并为它添加自己的 server |
