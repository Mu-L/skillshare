...
| **MCP** | 每个 server 一行，以及它同步到的 Agents 开关。**Add server** 接受 URL、命令、粘贴的片段或文件；**Import** 会读取某个已安装 Agent 现有的配置。每个 server 的菜单都有 **View what each Agent gets**，用于显示每个 Agent 的原生配置，包括尚未保存的编辑。冲突会提供 Import 或 Replace 选项，Backup 也可以在恢复前预览。对于使用 `pi-mcp-adapter` 的 Pi server，server 对话框中还有 **Direct tools**，以及一个 **其他 adapter 设置** 输入框，用于以 JSON 形式填写 [`piOptions`](./mcp.md#pi-options)。**默认值** 用于编辑 `mcp.targets` 和 `mcp.directTools`。 |
...
| **Targets** | 带状态的 Target 列表。每个 Target 的页面可编辑 include/exclude filter，并把本地专属的 skills 收集回 Source |
| **Projects** | 仅限 global mode。global 配置会 sync 进的项目文件夹，来自 [`projects`](/docs/reference/targets/configuration#projects) 和 [`mcp.projects`](./mcp.md#projects-in-the-dashboard)。**Add project** 需要填写文件夹、它的 Target 和要 sync 的内容。每个项目都有带 filter 的 **Skills** 和 **Agents** 标签页，可以预览并查看会写入的文件夹，还有一个 **MCP** 标签页，用于在该文件夹中关闭 global server 或为它添加自己的 server。已经指向某个项目文件夹的 Target 可以被转换 |
